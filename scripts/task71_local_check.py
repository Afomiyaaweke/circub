#!/usr/bin/env python3
"""Task 71 local E2E: budget ON the camera search result (v54).
...
def main():
    ...
    with sync_playwright() as p:
        ...
        scan_and_wait(page, "U-desktop", "task71-local-desktop")
        ...
        if toggle.count() > 0:
            ...
            page.screenshot(path=f"{SHOTS}/task71-local-dark-budget.png")
            goback = page.locator('button[aria-label="Go back"]').first
            if goback.count() > 0:
                goback.click()
            page.wait_for_timeout(1200)
        ...
        page_m.screenshot(path=f"{SHOTS}/task71-local-mobile-budget.png")
        page_m.close()
        browser.close()
